...
if __name__ == '__main__':
    n = Net()
    print(n)
    input = torch.unsqueeze(torch.linspace(-10, 10, 100), dim=1)
    ...
    for i in range(100):
        ...
        if i % 5 == 0:
            ...
            plt.text(-0.75, 2, 'Round=%d,Loss=%.4f' % (i,l.data[0]), fontdict={'size': 20, 'color':  'red'})
            plt.pause(0.3)
    plt.close()
    input2 = torch.unsqueeze(torch.linspace(-20, 20, 200), dim=1)
    ...
    plt.text(-3, 16, 'Round=%d,Loss=%.4f' % (100, l.data[0]), fontdict={'size': 20, 'color': 'red'})
    plt.show()
